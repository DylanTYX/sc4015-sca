from scipy.stats import pearsonr
import matplotlib.pyplot as plt
import numpy as np


def hamming_weight(int_no):
    """Return the number of set bits (1s) in an integer."""
    c = 0
    while int_no:
        int_no &= int_no - 1
        c += 1
    return c


Sbox = (
    ,
)

# 1. Load waveform.csv and extract plaintexts and power traces.
plaintexts = []
traces = []

with open("waveform.csv") as f:
    for line in f:
        # Drop empty values to handle occasional trailing commas safely.
        parts = [p for p in line.strip().split(",") if p]

        plaintext_hex = parts[0]
        # CSV format: plaintext, ciphertext, then power samples.
        trace = list(map(float, parts[2:]))

        plaintext_bytes = bytes.fromhex(plaintext_hex)

        plaintexts.append(list(plaintext_bytes))
        traces.append(trace)

# Convert lists to NumPy arrays for fast slicing and vectorized operations.
plaintexts = np.array(plaintexts, dtype=np.uint8)
traces = np.array(traces)

# Use only the first 100 traces as required by the assignment.
NUM_TRACES = 100
plaintexts = plaintexts[:NUM_TRACES]
traces = traces[:NUM_TRACES]

num_traces = traces.shape[0]
trace_len = traces.shape[1]

print(
    "Number of traces loaded: {}".format(num_traces),
    "Length of each trace: {}".format(trace_len),
)

# 2. CPA setup
NUM_KEYS = 256
NUM_BYTES = 16

recovered_key = []

# Store scores for the first key byte for plotting later (Plot-1 requirement).
scores_byte0 = None

# 3. CPA per byte
for byte_index in range(NUM_BYTES):
    print(f"\nProcessing key byte {byte_index}...")

    plaintext_column = plaintexts[:, byte_index]

    # Build hypothetical power model (256 x num_traces)
    power_model = np.zeros((NUM_KEYS, num_traces))

    for k_guess in range(NUM_KEYS):
        model = []

        for pt in plaintext_column:
            val = Sbox[pt ^ k_guess]
            model.append(hamming_weight(val))

        power_model[k_guess] = model

    # Correlation matrix: (256 x trace_len)
    C = np.zeros((NUM_KEYS, trace_len))

    for k in range(NUM_KEYS):
        for t in range(trace_len):
            corr, _ = pearsonr(power_model[k], traces[:, t])
            C[k, t] = abs(corr)

    # Score each key guess by its maximum correlation across all time samples.
    scores = np.max(C, axis=1)
    best_key = np.argmax(scores)

    recovered_key.append(best_key)

    print(
        f"Best guess for byte {byte_index}: {best_key} with score {scores[best_key]:.4f}"
    )

    # Save only first-byte scores for plotting.
    if byte_index == 0:
        scores_byte0 = scores.copy()

# 4. Final recovered key
print("\n========================")
print("\nRecovered Key:")
print([hex(k) for k in recovered_key])

# 5. Plot for project report (Plot-1 requirement).
plt.figure()

plt.plot(scores_byte0)
plt.scatter(
    np.argmax(scores_byte0), np.max(scores_byte0), color="red", label="Best Guess"
)

plt.title("CPA Correlation for Key Byte 0")
plt.xlabel("Key Guess (0-255)")
plt.ylabel("Max Correlation")
plt.legend()

plt.show()
